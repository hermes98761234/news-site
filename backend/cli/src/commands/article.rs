    ...
    match cmd {
        ArticleCmd::List { status } => {
            ...
            let res: serde_json::Value = client
                .get(&format!("/api/manage/articles{query}"))
                .await?;
        ...
        ArticleCmd::Publish { id } => {
            let res: serde_json::Value = client
                .post(
                    &format!("/api/manage/articles/{id}/publish"),
                    &serde_json::json!({}),
                )
                .await?;
            println!(
        ...
        ArticleCmd::Archive { id } => {
            let res: serde_json::Value = client
                .post(
                    &format!("/api/manage/articles/{id}/archive"),
                    &serde_json::json!({}),
                )
                .await?;
            println!(
